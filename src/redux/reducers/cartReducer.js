...
export const cartReducer = (store = initialState, {
    type,
    payload
}) => {
    switch (type) {
        case ADD_CART_ITEM:
            return {
                ...store, cartData: [...addToCart(store, payload)]
            }
            case DELETE_CART_ITEM:
                return {
                    cartData: [...deleteItem(store, payload)]
                }
                case INCREASE_ITEM_QUANTITY:
                    return {
                        cartData: [...incrementQuantity(store.cartData, payload)]
                    }
                    case DECREASE_ITEM_QUANTITY:
                        return {
                            cartData: [...decrementQuantity(store.cartData, payload)]
                        }
